...
class DashboardController {
  ...
  adminTimbangan = async (req, res) => {
    try {
      ...
      const totalWeightToday = await this.wasteCollectionService.getTotalWeight(startOfDay, now);
      const totalWeightThisMonth = await this.wasteCollectionService.getTotalWeight(startOfMonth, now);
  
      const wasteSummaryResult = await this.wasteSalesService.getWasteSummary(null, startOfMonthDateOnly, nowDateOnly);
  
      let salesData = [];
      let totalSalesAmount = 0;
      if (wasteSummaryResult.response && wasteSummaryResult.response.data && wasteSummaryResult.response.data.result) {
        salesData = wasteSummaryResult.response.data.result;
        totalSalesAmount = salesData.reduce((sum, item) => sum + item.total_price, 0);
      } else {
        console.log('Waste summary result structure is not as expected:', wasteSummaryResult);
      }
  ...
  try {
    ...
    const chartData = await this.wasteSalesService.getDetailChartData(waste_type_id, startDate, endDate);
...
getChartData = async (req, res) => {
  try {
    ...
    const chartData = await this.wasteSalesService.getChartData(startDate, endDate);
