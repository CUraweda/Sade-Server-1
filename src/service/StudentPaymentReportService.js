...
class StudentPaymentReportService {
  ...
  showStudentPaymentReportByFilter = async (
    classes,
    student,
    start_date,
    end_date,
    ...
    let cl = await this.studentPaymentReportDao.getByFilter(
      classes,
      student,
      start_date,
      end_date,
  ...
  async showPage(page, limit, search, offset, filters) {
    const totalRows = await this.studentPaymentReportDao.getCount(
      search,
      filters
    );
  ...
  async exportPage(search, filters) {
    ...
    const sheetData = result
      .map((dat, i) => ({
        ...
        Total: dat.studentpaymentbill?.total ?? 0,
        Status: dat.status.toUpperCase(),
        "Tanggal Bayar": dat.paidoff_at
          ? moment(dat.paidoff_at).format("YYYY-MM-DD HH:mm:ss")
          : "",
      }));
